...
export const Cart = () => {
  ...
  const increaseQuantity = async (item) => {
    try {
      ...
      const updatedItems = cartItems.map((cartItem) => (cartItem.id === item.id ? updatedItem : cartItem));
      setCartItems(updatedItems);
  ...
  const placeOrder = async () => {
    try {
      const total = cartItems.reduce((sum, item) => sum + parseFloat(item.price), 0); // Tính tổng số tiền
